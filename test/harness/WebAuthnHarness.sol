...
contract WebAuthnHarness is WebAuthnValidator {
    ...
    function debug_webAuthVerify(bytes32 txHash, bytes calldata fatSignature)
        external
        view
        returns (bool)
    {
    ...
    function debug_checks(bytes32 txHash, bytes calldata fatSignature)
        external
        view
        returns (bool rsOk, bool flagsOk, bool challengeOk, bool typeOk, bool keyOk, bool crossOk, bytes32 message)
    {
        ...
        flagsOk = authenticatorData.length > 32 && (authenticatorData[32] & AUTH_DATA_MASK_LOCAL == AUTH_DATA_MASK_LOCAL);
