...
function resolveBroadcastFile() {
  const broadcastDir = join(__dirname, "../../broadcast/Deploy.s.sol");
  const requestedChainId = process.env.CHAIN_ID;
  const candidateFileNames = ["deployAll-latest.json", "run-latest.json"];

  const candidateDirs = requestedChainId
    ? [join(broadcastDir, requestedChainId)]
    : readdirSync(broadcastDir, { withFileTypes: true })
        .filter((entry) => entry.isDirectory())
        .map((entry) => join(broadcastDir, entry.name))
        .sort(
  ...
  throw new Error(
    `Could not find a Deploy.s.sol broadcast file in ${broadcastDir}`
  );
}

export function contractAddresses() {
  const txs = require(resolveBroadcastFile()).transactions;
  return {
...
export function toPasskeySigner(
  privateKey: crypto.KeyObject,
  credentialId: Hex
) {
  const { webauthnValidator } = contractAddresses();
  return async function (userOpHash: Hex) {
    const signature = signWithPasskey(
...
if (hashIndex !== -1) {
  ...
  const privateKeyJSON: crypto.JsonWebKey = {
    ...
    crv: "P-256",
    d: "mVCuyGzW2iB7wl2Lt3waJHHzWmtc6W-lzzIvjgHVA8U",
  };
