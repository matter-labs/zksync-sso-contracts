...
contract SessionKeyValidator is IValidator {
  ...
  function isBannedCall(address target, bytes4 /* selector */) internal view virtual returns (bool) {
    return
      ...
      IMSA(msg.sender).isModuleInstalled(MODULE_TYPE_VALIDATOR, target, "");
  ...
  function validateUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash) public virtual returns (uint256) {
    ...
    // console.logBytes32(userOp);
    (bytes memory transactionSignature, SessionLib.SessionSpec memory spec, uint48[] memory periodIds) = abi.decode(
      userOp.signature[20:],
      (bytes, SessionLib.SessionSpec, uint48[])
    );
    console.log("decode successful");
    require(spec.signer != address(0), SessionLib.ZeroSigner());
    console.log("signer not zero");
    ...
    console.log("nonceKey check successfull");
    ...
    if (
      recoverError != ECDSA.RecoverError.NoError || recoveredAddress == address(0) || recoveredAddress != spec.signer
    ) {
    ...
    console.log("validateFeeLimit successful");
