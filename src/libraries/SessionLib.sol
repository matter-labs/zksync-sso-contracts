...
library SessionLib {
  ...
  function checkAndUpdate(
    ...
    if (
      ...
      console.log("ConditionViolated");
      revert ConditionViolated(param, refValue, uint8(condition));
    }
    console.log("conditions check success");
  ...
  function validateFeeLimit(
    ...
    if (userOp.paymasterAndData.length == 0) {
      ...
      console.log("fee:", fee);
      console.log("feeLimit:", spec.feeLimit.limit);
      // slither-disable-next-line unused-return
  ...
  function validate(
    ...
    console.log("session status check success");
    ...
    console.log("call type check success");
    require(
    ...
    console.log("selector check success");

    // TODO: put a comment about why this exact slice
    uint256 length = uint256(bytes32(userOp.callData[68:100]));
    (address target, uint256 value, bytes calldata callData) = ExecutionLib.decodeSingle(
      userOp.callData[100:100 + length]
    );
    console.log("decodeSingle success");
    ...
    if (callData.length >= 4) {
      ...
      console.log("found check success");
      require(value <= callPolicy.maxValuePerUse, MaxValueExceeded(value, callPolicy.maxValuePerUse));
      console.log("value <= callPolicy.maxValuePerUse check success");
      (uint48 newValidAfter, uint48 newValidUntil) = callPolicy.valueLimit.checkAndUpdate(
        state.callValue[target][selector],
        value,
        periodIds[1]
      );
      ...
      console.log("maxValuePerUse:", transferPolicy.maxValuePerUse);
      ...
      console.log("value <= callPolicy.maxValuePerUse check success");
      (uint48 newValidAfter, uint48 newValidUntil) = transferPolicy.valueLimit.checkAndUpdate(
        state.transferValue[target],
        value,
        periodIds[1]
      );
      console.log("valueLimit check success");
      shrinkRange(timeRange, newValidAfter, newValidUntil);
    }
